...
class UuagentValidator(Validator):
    ...
    def validate(self, raw_str):
        ...
        #type check
        rf_type = json_obj.get("rf_type")
        if rf_type not in ["uhd", "zmq"]:
            self.errors.append("rf_type must be 'uhd' or 'zmq'")
        #frequency check

        rx_freq = json_obj.get("rf_rx_freq")
        if isinstance(rx_freq, (int, float)):
            if rx_freq <= 0:
                self.errors.append("rf_rx_freq 0 error")
            else:
                in_fr1 = 410e6 <= rx_freq <= 7125e6
                in_fr2 = 24.25e9 <= rx_freq <= 52.6e9
                if not (in_fr1 or in_fr2):
                    self.errors.append("rf_rx_freq must be inside NR FR1 (410e6–7.125e9) or FR2 (24.25e9–52.6e9)")
        #sample rate check
        srate = json_obj.get("rf_srate")
        if isinstance(srate, (int, float)) and srate <= 0:
            self.errors.append("rf_srate must be > 0")
        #gain checks
        rx_gain = json_obj.get("rf_rx_gain")
        if isinstance(rx_gain, (int, float)):
            if not (0 <= rx_gain <= 90):
                self.errors.append("rf_rx_gain must be between 0 and 90 dB")

        tx_gain = json_obj.get("rf_tx_gain")
        if isinstance(tx_gain, (int, float)):
            if not (0 <= tx_gain <= 90):
                self.errors.append("rf_tx_gain must be between 0 and 90 dB")
        
        num_samples = json_obj.get("rf_num_samples")
        if isinstance(num_samples, int):
            if num_samples <= 0:
                self.errors.append("rf_num_samples must be > 0")
            elif num_samples > 200:
                self.errors.append("rf_num_samples must be <= 200")

        #validate file path
        iq_file = json_obj.get("rf_iq_file", "")
        if not iq_file:
            self.errors.append("rf_iq_file cannot be empty")
        elif not iq_file.startswith("/output/"):
            self.errors.append("rf_iq_file should be in /output/ directory for docker deployment")

        #hardware specific for uhd type and b200
        device_args = json_obj.get("rf_device_args", "")
