...
class decoder:
    ...
    def start(self, process_config):
        # TODO: configure based on how decoder works

        # setting up
        self.decoder_config = process_config["config_file"]
        self.container_name = process_config["id"]
        self.decoder_args = process_config["args"] if "args" in process_config.keys() else [""]
        ...
        if rf_config["type"] == "b200":
            ...
            raise RuntimeError(f"Invalid RF type for decoder: {rf_config['type']}")
